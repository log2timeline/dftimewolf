...
class GoogleCloudDiskExportTest(unittest.TestCase):
  ...
  def testSetUp(self, mock_gcpProject, mock_getDisk):
    """Tests that the collector can be initialized."""

    test_state = state.DFTimewolfState(config.Config)
    disk_export_collector = gce_disk_export.GoogleCloudDiskExport(
        test_state)
    mock_gcpProject.return_value = FAKE_SOURCE_PROJECT
    FAKE_SOURCE_PROJECT.GetDisk = mock_getDisk
  ...
  def testProcess(self,
    ...
    FAKE_SOURCE_PROJECT.CreateImageFromDisk = mock_createImageFromDisk
    mock_createImageFromDisk.return_value = FAKE_IMAGE
    FAKE_IMAGE.ExportImage = mock_exportImage
    FAKE_IMAGE.Delete = mock_deleteImage
    disk_export_collector.Process()
    mock_createImageFromDisk.assert_called_with(
        FAKE_DISK, name='{0:s}-image-df-export-temp'.format(
            'fake-source-disk'))
    mock_exportImage.assert_called_with(
        'gs://fake-bucket', output_name='{0:s}-image-df-export-temp'.format(
            'fake-source-disk'))
    mock_deleteImage.assert_called_once()
