...
INVALID_SHEET = {
    ...
    'majorDimension':
        'ROWS',
    'values': [[
        'message', 'timestamp', 'timestamp_desc ', 'extra_field_1 ',
        'extra_field_2'
    ], ['A message', '1331698658276340', 'Write time', 'foo ', 'bar'],
               ['', '1331698658276340', 'create', 'dodo'],
               ['sdsadasd', '', 'ddd', 'dodo', 'd']]
}
VALID_SHEET = {
    'range':
    ...
    'majorDimension':
        'ROWS',
    'values': [[
        'message', 'timestamp', 'datetime', 'timestamp_desc ', 'extra_field_1 ',
        'extra_field_2'
    ],
               [
                   'A message', '1331698658276340', '2015-07-24T19:01:01+00:00',
                   'Write time', 'foo ', 'bar'
               ],
               [
                   '', '1331698658276340', '2016-07-24T19:01:01+00:00',
                   'create', 'dodo'
               ], ['sdsadasd', '', '', 'ddd', 'dodo', 'd']]
}
...
class GoogleSheetsCollectorTest(unittest.TestCase):
  ...
  def testParseSheet(self, _mock_discovery):
    """Tests that the collector can parse valid and returns None for invalid sheets."""
    test_state = state.DFTimewolfState(config.Config)
    collector = gsheets.GoogleSheetsCollector(test_state)
    ...
    self.assertIsNotNone(collector._ParseSheet(spreadsheet_id, sheet_title))

    service.spreadsheets.return_value.values.return_value.get.return_value.execute.return_value = INVALID_SHEET

    self.assertIsNone(collector._ParseSheet(spreadsheet_id, sheet_title))
