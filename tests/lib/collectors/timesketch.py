...
class TimesketchSearchEventCollectorTest(unittest.TestCase):
  ...
  def testSetupWithToken(self, mock_GetApiClient):
    ...
    mock_GetApiClient.assert_called_with(
        test_state, token_password='test_token')

  @mock.patch('dftimewolf.lib.timesketch_utils.GetApiClient')
  @mock.patch('timesketch_api_client.client.TimesketchApi')
  def testSetupWithUsername(self, mock_timesketchApi, _mock_GetApiClient):
    ...
    self.assertEqual(timesketch_collector.start_datetime, datetime.datetime(2024, 11, 11))
    self.assertEqual(timesketch_collector.end_datetime, datetime.datetime(2024, 11, 12))
    ...
    mock_timesketchApi.assert_called_with('127.0.0.1', 'user', 'pass')

  @mock.patch('dftimewolf.lib.timesketch_utils.GetApiClient')
  @mock.patch('timesketch_api_client.search')
  def testSetup(self, mock_search, mock_GetApiClient):
    """Tests the SetUp function with token."""
    test_state = state.DFTimewolfState(config.Config)
    timesketch_collector = timesketch.TimesketchSearchEventCollector(
        test_state)
  ...
  @mock.patch.object(
      ...
      '_GetSearchResults')
  def testProcessPandas(self, mock_GetSearchResults, _mock_GetApiClient):
    """Test the Process function with Pandas output."""
    mock_GetSearchResults.return_value = pd.DataFrame([1, 2])
    test_state = state.DFTimewolfState(config.Config)
    timesketch_collector = timesketch.TimesketchSearchEventCollector(test_state)
    timesketch_collector.SetUp(
        sketch_id='1',
