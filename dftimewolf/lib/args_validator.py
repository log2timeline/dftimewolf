...
"""
Yeah I think that's what I meant, unless I misunderstand you. I was thinking you'd end up with
argsparse.add_argument(...., type=validator[recipe][switch])
That dict lookup returning a callable
from an already instantiated object based on what's in the final item in the arg list definition in the recipe (the format piece)


Thomas Chopitea, Yesterday 19:16
hahahaha oh god XD

Thomas Chopitea, Yesterday 19:19
but yeah that's about right. or even 

for arg, desciption, default, options in args:
  argsparse.add_argument(...., type=dftwValdators[options['format']](options))

but it would make it harder to do what you wanted to do later on with comparing one arg to the other
although this can be achieved with actions in argparse
but probably in a more convoluted way

Damian Johnstone, Yesterday 19:22
The value passed to type needs to be Callable(str)
"""
