...
class TimesketchExporter(BaseExporter):
  ...
  def __init__(self, auth, incident_id, sketch_id, verbose, paths):
    """Initializes a filesystem collector.

    Args:
      auth: an (endpoint, username, password) tuple
      incident_id: Incident ID or description associated with the investigation
    ...
    ts_endpoint, ts_username, ts_password = auth
  ...
  def launch_exporter(
    ...
    auth = (ts_endpoint, ts_username, ts_password)
    exporter = TimesketchExporter(
        auth, incident_id, sketch_id, verbose, processor_output)
    exporter.start()
    return [exporter]
