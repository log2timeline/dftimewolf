...
class TimesketchExporter(module.BaseModule):
    ...
    def _WaitForTimelines(self):
        ...
        time.sleep(
            5)  # Give Timesketch time to populate recently added timelines.
        sketch = self.timesketch_api.get_sketch(self.sketch_id)
    ...
    def Process(self):
        ...
        if not sketch and self.sketch_id:
            self.logger.info(
                'Using exiting sketch: {0:d}'.format(self.sketch_id))
            sketch = self.timesketch_api.get_sketch(self.sketch_id)
        ...
        with importer.ImportStreamer() as streamer:
            ...
            for dataframe_container in self.state.GetContainers(containers.DataFrame):
                streamer.set_timeline_name(dataframe_container.name)
                streamer.add_data_frame(dataframe_container.data_frame)
                streamer.timeline.description = dataframe_container.description
