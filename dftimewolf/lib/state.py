...
class DFTimewolfState(object):
  ...
  def __init__(self,
               config: Type[Config],
               cursesdm: Optional[cdm.CursesDisplayManager] = None) -> None:
    """Initializes a state."""
  ...
  def LoadRecipe(self,
    ...
    if self.cdm:
      self.cdm.SetRecipe(self.recipe['name'])
      for module_definition in preflight_definitions:
        self.cdm.EnqueuePreflight(module_definition['name'],
                                  module_definition['wants'],
                                  module_definition.get('runtime_name'))
      for module_definition in module_definitions:
        self.cdm.EnqueueModule(module_definition['name'],
                               module_definition['wants'],
                               module_definition.get('runtime_name'))
      self.cdm.Draw()
    ...
    try:
      if isinstance(module, ThreadAwareModule):
        ...
        logger.info(
            ...
            f'simultaneous for module {runtime_name}')
